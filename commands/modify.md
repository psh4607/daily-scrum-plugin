...
CronList 도구로 등록된 스케줄 목록을 조회한다.

이름에 `daily-scrum`이 포함된 스케줄을 필터링한다.

### 1-2. 결과 분기

**스케줄을 찾은 경우:**

스케줄의 프롬프트 내용에서 `## 설정 정보` 섹션을 파싱하여 현재 설정값을 추출한다:
...
6. 컨펌 후 → **스케줄 재생성** (아래 "스케줄 재생성 패턴" 참조).
...
5. 컨펌 후 → 스케줄 프롬프트의 `CHANNELS_CONFIG`에 새 채널 정보를 추가하고, `CHANNEL_NAMES`에 채널 이름을 추가하여 **스케줄 재생성**.
...
5. 컨펌 후 → 기존 CronDelete → 새 cron 표현식으로 **CronCreate** (프롬프트의 REPLY_TIME도 갱신).
...
4. 컨펌 후 → 프롬프트의 `LINEAR_SCOPE_DESC`와 `LINEAR_SCOPE_FILTER`를 갱신하여 **스케줄 재생성**.
...
4. 컨펌 후 → 프롬프트의 `HOLIDAY_KEYWORDS`를 갱신하여 **스케줄 재생성**.
...
1. 유저에게 새 이름을 입력받는다.
   ...
   > 새 표시 이름을 알려주세요. (현재: {현재 이름})

2. 변경 내용을 유저에게 보여주고 컨펌을 받는다.
...
3. 컨펌 후 → 프롬프트의 `USER_NAME`을 갱신하여 **스케줄 재생성**.

---

### G) 스케줄 일시 중지 / 재개

현재 스케줄 상태에 따라 동작이 달라진다.
...
2. 컨펌 후 → CronDelete로 스케줄을 삭제한다.

3. 삭제 전 프롬프트 내용과 cron 표현식, 스케줄 이름을 기록해둔다. 유저에게 안내한다:

   > ⏸️ 스케줄이 일시 중지되었습니다.
   >
   > 재개하려면 `/daily-scrum:modify`를 다시 실행하세요.

   이 경우 프롬프트와 cron 정보를 유저에게 표시하여, 재개 시 그대로 CronCreate할 수 있도록 한다.

**재개 (스케줄이 비활성 상태일 때 — 즉 CronList에 daily-scrum 스케줄이 없을 때):**

일시 중지된 스케줄을 재개하려면 프롬프트 내용이 필요하다. CronList에 스케줄이 없으므로 Step 1에서 "스케줄을 찾지 못한 경우"로 분기되어 `/daily-scrum:setup`을 안내한다.

따라서 일시 중지 시 유저에게 프롬프트와 cron 정보를 반드시 보여주고, 재개 시에는 setup을 다시 실행하거나 유저가 보관한 정보를 기반으로 수동 CronCreate하도록 안내한다.
...
1. 유저에게 확인한다.
   ...
   > ⚠️ 정말 삭제하시겠습니까? 삭제하면 프롬프트 설정도 함께 사라집니다.
   > 다시 사용하려면 `/daily-scrum:setup`으로 처음부터 설정해야 합니다.

2. 유저가 명시적으로 삭제를 확인하면 CronDelete로 스케줄을 삭제한다.

3. 완료 안내:

   > 🗑️ 스케줄 "{스케줄 이름}"이 삭제되었습니다.
...
수정 항목 A~F에서 설정이 변경되면 스케줄을 재생성한다. 재생성 절차:

### 설정 변경 시 (cron 시각 변경 없음)

1. Read 도구로 `prompts/schedule-template.md`를 읽는다. 경로는 이 파일(commands/modify.md) 기준 `../prompts/schedule-template.md`이다.
2. 변경된 설정값으로 플레이스홀더를 치환한다. 변경되지 않은 값은 기존 스케줄에서 파싱한 값을 그대로 사용한다.
3. CronDelete로 기존 스케줄을 삭제한다.
4. CronCreate로 새 스케줄을 등록한다:
   - **이름**: 기존과 동일 (예: `daily-scrum-{채널이름}`). 채널이 변경된 경우 `daily-scrum-{새채널이름}`으로 갱신한다.
   - **cron 표현식**: 기존과 동일.
   - **프롬프트**: 플레이스홀더가 치환된 schedule-template.md 전체 내용.

### cron 시각 변경 시 (항목 C)

1. Read 도구로 `prompts/schedule-template.md`를 읽는다.
2. 변경된 설정값으로 플레이스홀더를 치환한다.
3. CronDelete로 기존 스케줄을 삭제한다.
4. 새 답글 시각의 KST → UTC 변환 결과로 cron 표현식을 새로 생성한다.
5. CronCreate로 새 스케줄을 등록한다:
   - **이름**: 기존과 동일.
   - **cron 표현식**: 새로 계산한 UTC 기반 cron.
   - **프롬프트**: 플레이스홀더가 치환된 schedule-template.md 전체 내용.
