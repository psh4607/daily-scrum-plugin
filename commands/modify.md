...
3. 컨펌 후 → 프롬프트의 `USER_NAME`을 갱신하여 **스케줄 업데이트** (아래 "스케줄 업데이트 패턴" 참조).
...
수정 항목 A~F에서 설정이 변경되면 `RemoteTrigger action:"update"`로 기존 트리거를 수정한다. 삭제 후 재생성할 필요 없이 partial update가 가능하다.
...
3. `RemoteTrigger action:"update"` trigger_id:{trigger_id}로 트리거를 수정한다. body에는 변경이 필요한 필드만 포함한다:
   - **name**: 채널이 변경된 경우 `daily-scrum-{새채널이름}`으로 갱신. 그 외에는 생략.
   - **job_config**: 프롬프트가 변경된 경우 갱신된 프롬프트로 `job_config.ccr.events[0].data.message.content`를 업데이트.

### cron 시각 변경 시 (항목 C)

1. Read 도구로 `prompts/schedule-template.md`를 읽는다.
2. 변경된 설정값으로 플레이스홀더를 치환한다.
3. 새 답글 시각의 KST → UTC 변환 결과로 cron 표현식을 새로 생성한다.
4. `RemoteTrigger action:"update"` trigger_id:{trigger_id}로 트리거를 수정한다. body:
   - **cron_expression**: 새로 계산한 UTC 기반 cron.
   - **job_config**: 갱신된 프롬프트(REPLY_TIME 반영)로 업데이트.
